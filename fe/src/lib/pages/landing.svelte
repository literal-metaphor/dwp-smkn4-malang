      ...
      {#if !$authStatus}
        <div class={`flex items-center p-4 space-x-4`}>
            <button on:click={() => {
              isRegister = false;
              showAuthModal();
            }} use:ripple class="bg-white px-6 py-2 font-semibold rounded-md border border-grey">Masuk</button>
            <button on:click={() => {
              isRegister = true;
              showAuthModal();
            }} use:ripple class="bg-french-violet text-white font-semibold px-6 py-2 rounded-md">Daftar</button>
        </div>
      {:else}
        <button on:click={() => {
          sessionPage.set("beranda");
        }} use:ripple class="bg-french-violet text-white font-semibold px-6 py-2 rounded-md">Beranda</button>
      {/if}
  </div>
</header>
...
<section id="hero" class="bg-background py-20">
  <div class="lg:container w-[90vw] lg:w-[50vw] mx-auto text-center">
      ...
      <h1 class="text-h6 font-bold mb-6">Temukan Kebutuhan Rumah Tangga dan Banyak Lagi <br> Hanya di <span class="text-french-violet">DWP SMKN 4 Malang</span></h1>

      <p class="mb-6">Rasakan sensasi berbelanja tanpa rasa khawatir. Nikmati kumpulan produk kami yang berkualitas tinggi dengan harga terjangkau.</p>

      {#if !$authStatus}
        <button on:click={showAuthModal} use:ripple class={`m-2 bg-french-violet text-white font-semibold px-12 py-4 mx-1 rounded-md`}>Gabung Sekarang</button>
      {:else}
        <button on:click={() => {
          sessionPage.set("beranda");
        }} use:ripple class={`m-2 bg-french-violet text-white font-semibold px-12 py-4 mx-1 rounded-md`}>Pergi ke Beranda</button>
      {/if}
      <button use:ripple on:click={() => document.getElementById("produk_kami")?.scrollIntoView({ behavior: "smooth" })} class={`m-2 bg-white px-12 py-4 font-semibold rounded-md border border-grey mx-1`}>Lihat Lebih Lanjut</button>
  </div>
</section>
...
<section id="keunggulan" class="py-12 p-4 bg-white">
  <div class="lg:container mx-auto">
    ...
    <div class="grid grid-cols-3 gap-6">
      <div class="col-span-3 lg:col-span-1 bg-white p-6 rounded-md shadow-md flex justify-between items-center">
...
<section class="py-12 p-4 bg-french-violet text-white">
  <div class="lg:container mx-auto flex flex-col items-center lg:items-start">
    ...
    <br>
    {#if !$authStatus}
      <button on:click={showAuthModal} use:ripple class="bg-white text-black px-8 py-3 rounded-md">Gabung Sekarang</button>
    {:else}
      <button on:click={() => {
        sessionPage.set("beranda");
      }} use:ripple class="bg-white text-black px-8 py-3 rounded-md">Belanja Sekarang</button>
    {/if}
    <!-- <img src="" alt="CTA"> -->
  </div>
</section>
