<script lang="ts">
	...
	onMount(async () => {
		const res = await api.get(`/transaction/user/${JSON.parse(localStorage.getItem('userData') || '{}').id}`);
	...
	function filterByDate(e: Event) {
		...
		filteredTransactions = transactions.filter((transaction) => transaction.created_at.includes(target.value));
	}
	...
	async function confirmDelivery(id: string) {
		if (confirm('Apakah Anda yakin ingin mengkonfirmasi pengiriman ini?')) {
			...
			try {
				const res = api.post(`/transaction/confirm/${id}`, formData, { headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}` } });
				location.reload();
			} catch (err) {
				console.log(err);
				switch (true) {
...
<dialog id="transaction_modal" class="daisy-modal">
	...
	<br>
		...
		use:ripple
		type="button"
		on:click={() => setTimeout(() => open(`https://www.google.com/maps/search/${currentTransactionData.latitude},${currentTransactionData.longitude}/@${currentTransactionData.latitude},${currentTransactionData.longitude}m/data=!3m1!1e3?entry=ttu`), 250)}
		class="flex justify-center items-center bg-white shadow-lg p-4 rounded-lg w-full"
	>
	<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="14.32 4.87961494 37.85626587 52.79038506" width="24"><path d="m37.34 7.82c-1.68-.53-3.48-.82-5.34-.82-5.43 0-10.29 2.45-13.54 6.31l8.35 7.02z" fill="#1a73e8"/><path d="m18.46 13.31a17.615 17.615 0 0 0 -4.14 11.36c0 3.32.66 6.02 1.75 8.43l10.74-12.77z" fill="#ea4335"/><path d="m32 17.92a6.764 6.764 0 0 1 5.16 11.13l10.52-12.51a17.684 17.684 0 0 0 -10.35-8.71l-10.51 12.51a6.74 6.74 0 0 1 5.18-2.42" fill="#4285f4"/><path d="m32 31.44c-3.73 0-6.76-3.03-6.76-6.76a6.7 6.7 0 0 1 1.58-4.34l-10.75 12.77c1.84 4.07 4.89 7.34 8.03 11.46l13.06-15.52a6.752 6.752 0 0 1 -5.16 2.39" fill="#fbbc04"/><path d="m36.9 48.8c5.9-9.22 12.77-13.41 12.77-24.13 0-2.94-.72-5.71-1.99-8.15l-23.57 28.05c1 1.31 2.01 2.7 2.99 4.24 3.58 5.54 2.59 8.86 4.9 8.86s1.32-3.33 4.9-8.87" fill="#34a853"/></svg>
		...
		{#each currentTransactionData.items as item}
			<div class="bg-white p-4 shadow-lg rounded-lg my-4 relative">
				...
				<div class="flex items-center mb-4">
						<img src={item.product?.owner?.avatar?.filename ? store + item.product?.owner?.avatar?.filename : userPlaceholder} alt="Profile" class="size-10 rounded-full me-2">
				...
				<div class="flex-1">
						...
						<div class="mt-4">
							...
							<div class="font-bold">Total:
								<span class="font-normal">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(Number(item.product?.price) * item.quantity)}</span>
							</div>
						</div>
						...
						<!-- Delivery Status -->
						<div class={`flex items-center text-${item.status === 'delivered' ? 'success' : 'red'}`}>
...
<div class={`lg:container overflow-x-hidden flex lg:items-center flex-col w-screen h-fit p-4`}>
	...
	<div class="flex items-center"><span class="me-2">Filter pada tanggal:</span> <input on:change={filterByDate} type="date" name="date_search" id="date_search"></div>
		...
		{#each filteredTransactions as data}
			...
			>
				...
				<div class="flex flex-col items-end">
					...
					<p class="text-sm text-french-violet">{new Date(data.created_at).toISOString().split('T')[0]}</p>
				</div>
			</button>
		{/each}
	</div>
