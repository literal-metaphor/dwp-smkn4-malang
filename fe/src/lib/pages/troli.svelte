<script lang="ts">
  ...
  async function finalizeCodDelivery(e: SubmitEvent) {
    ...
    try {
      await api.post(`/transaction`, data, { headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}` } });
...
<dialog id="cod_modal" class="daisy-modal">
  ...
  <form on:submit={finalizeCodDelivery}>
    {#each [
      { name: 'customer_name', label: 'Nama Lengkap Anda', placeholder: 'Nama Lengkap Anda' },
      { name: 'area', label: 'Nama Perumahan / Jalan / Desa', placeholder: 'Nama Perumahan / Jalan / Desa' },
      { name: 'district', label: 'Kecamatan', placeholder: 'Kecamatan' },
      { name: 'city', label: 'Kabupaten / Kota', placeholder: 'Kabupaten / Kota' },
      { name: 'address_criteria', label: 'Petunjuk Lokasi', placeholder: 'Jelaskan kriteria lokasi Anda untuk mempermudah kurir dalam mencari lokasi pengiriman... (contoh: "dekat kantor kecamatan, jalan raya, rumah dengan pohon jambu, dll.")' },
    ] as field}
    <div class="mb-4">
...
<div class="pt-32 p-6">
  ...
  <div class="bg-white p-4 shadow-lg mb-6 rounded-lg">
      ...
      <button disabled={items.length < 1} on:click={openCodModal} use:ripple type="button" class={`p-4 rounded-lg border border-french-violet flex items-center ${items.length < 1 && 'cursor-not-allowed opacity-50'}`}>
