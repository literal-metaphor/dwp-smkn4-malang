<script lang="ts">
	...
	async function getFavoritedStatus() {
		try {
			const res = await api.get(
				`/product/wishlist/${JSON.parse(localStorage.getItem('userData') || '{}').id}/${product.id}`
			);
			isFavorite = res.data;
		} catch (err) {
			handleError(err);
	...
	async function getYourRating() {
		try {
			const res = await api.get(`/rating/user/${product.id}`, {
				headers: {
		...
		try {
			...
			if (carts && carts.length > 0) {
				if (carts.some((item) => item.product.id === product.id))
					throw new Error('Produk ini sudah ada di keranjang belanja');
			}
	...
	async function rateProduct(newRating: number) {
		try {
			const res = await api.post(
				`/rating`,
				{ product_id: product.id, rating: newRating },
				{
					headers: {
						Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}`
					}
	...
	let price: string = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(
		product.price
	);
	...
	async function updateProduct(e: SubmitEvent) {
		...
		try {
			...
			const res = await api.post(`/product/${product.id}`, formData, {
				headers: {
					Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}`
				}
			});
			if (res.status.toString().startsWith('2')) {
				...
				if (product.images && product.images.length > 0) {
					try {
						api.delete(`/product/photo/${product.id}`, {
							headers: {
								Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}`
							}
						});
				...
				const res = await api.post(`/product/photo/${product.id}`, fileFormData, {
					headers: {
						Authorization: `Bearer ${JSON.parse(localStorage.getItem('userData') || '{}').remember_token}`
					}
				});
			...
			<form on:submit={updateProduct}>
				...
				<div class="mb-4">
					...
					<select
						...
						required
					>
						{#each [{ value: 'food', label: 'Makanan' }, { value: 'drink', label: 'Minuman' }, { value: 'female_fashion', label: 'Fashion Wanita' }, { value: 'male_fashion', label: 'Fashion Pria' }, { value: 'child_fashion', label: 'Fashion Anak' }, { value: 'furniture', label: 'Perabotan' }] as category}
							<option selected={product.category === category.value} value={category.value}>
								{category.label}
							</option>
						{/each}
					</select>
				...
				<div class="mb-4">
					...
					<img
						id="preview"
						class="size-32 my-2"
						src={product.images && product.images.length > 0 ? store + product.images[0] : noimage}
						alt="Preview"
					/>
					<input
				...
				py-2 px-4 rounded focus:outline-none focus:shadow-outline ${loading && `opacity-50`} transition duration-300`}
					>Edit Produk</button
				>
				<button
					disabled={loading}
...
>
	<div class="flex justify-center items-center">
		<button
			on:click={() => sessionPage.set(isOwner ? `toko` : `beranda`)}
			type="button"
			class="me-2"
		>
			<svg
	...
	<div class="flex justify-center items-center">
		{#if isOwner}
			<button
				on:click={openUpdateProductModal}
				use:ripple
				type="button"
				class="me-2 p-2 rounded-full"
			>
	...
	<div class="lg:w-1/2 px-4">
		...
		<div class="flex items-center">
			...
			<span
				>⭐ {ratings && ratings.length > 0
					? (ratings.reduce((a, b) => a + b.rating, 0) / ratings.length / 5) * 100 + '%'
					: `0%`} ({ratings.length} ulasan)</span
			>
		</div>
		...
		{#if !isOwner}
			<div class="flex items-center mt-4">
				...
				<button
					on:click={() => addToCart()}
					use:ripple
					class="bg-french-violet text-white font-bold py-2 px-4 rounded ml-4"
					>🛒 Tambahkan ke Troli</button
				>
			</div>
		{/if}
		...
		<div class="mt-8 border-t pt-4">
			...
			<div class="flex items-center space-x-4">
				<img
					src={product.owner?.avatar ? store + product.owner.avatar.filename : userPlaceholder}
					alt="Shop"
					class="w-12 h-12 rounded-full"
				/>
				<span class="ml-2 text-lg font-bold text-blue"
					>{product.owner ? '@' + product.owner.username : `Loading...`}</span
				>
			</div>
		</div>
		...
		<div class="mt-8 border-t pt-4">
			...
			<div class="flex mt-2">
				...
				{#each Array(5) as _, i}
					...
					>
						<img
							src={i < yourRating ? starfill : star}
							alt="Rating indicator icon"
							class="size-full"
						/>
					</button>
				{/each}
			...
			<form on:submit={submitComment}>
				<textarea
					...
					placeholder="Tuliskan komentarmu di sini..."
				></textarea>
				<div class="flex items-center justify-start space-x-4 mt-2">
					<button
						type="submit"
						use:ripple
						class="bg-french-violet text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
						>Kirim</button
					>
					<button
						type="reset"
						use:ripple
						class="bg-white text-black border border-grey font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
						>Reset</button
					>
				</div>
			</form>
			...
			<div class="mt-4 h-64 overflow-y-auto">
				...
				{#each comments as data}
					...
					<div class="border-t py-2">
						...
						<p class="font-bold">
							<span class="text-blue">@{data.user && data.user.username}</span> - {(() => {
								const days = Math.floor(
									(new Date().getTime() - new Date(data.created_at).getTime()) / (1000 * 3600 * 24)
								);
								return days === 0
									? 'Today'
									: days > 365
