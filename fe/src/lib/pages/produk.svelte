              ...
              <!-- Replace the following with your comment component -->
               ...
               <!-- data because comment is currently used in PostCSS -->
                <div class="border-t py-2">
                    ...
                    <p class="font-bold"><span class="text-blue">@{data.user && data.user.username}</span> - {(() => { const days = Math.floor((new Date().getTime() - new Date(data.created_at).getTime()) / (1000 * 3600 * 24)); return days > 365 ? `${Math.floor(days / 365)} tahun yang lalu` : days > 30 ? `${Math.floor(days / 30)} bulan yang lalu` : `${days} hari yang lalu`; })()}</p>
                    <p>{data.content}</p>
                </div>
              {/each}
