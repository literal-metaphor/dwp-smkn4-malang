<script lang="ts">
  export let items: { img: string; alt: string }[];
...
<div class="relative">
  <div class="overflow-hidden relative w-[30vw] h-[40vw] rounded-lg">
    {#each items as item, index}
      <div class="absolute inset-0 transition-transform duration-700 ease-in-out transform {current === index ? 'translate-x-0' : 'translate-x-full'}">
        <img src={item.img} alt={item.alt} class="block w-full h-full object-cover" />
      </div>
    {/each}
  </div>
</div>