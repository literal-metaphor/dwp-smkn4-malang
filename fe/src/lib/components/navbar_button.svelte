...
<button use:ripple class={`rounded-full size-16 p-4 mx-1 ${isActive ? `bg-french-violet text-white`: ``} flex justify-center items-center flex-col active:scale-90 transition duration-300`} on:click={() => { sessionPage.set(option); window.scrollTo(0, 0); }}>
